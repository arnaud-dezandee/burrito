...
func (r *Reconciler) HasStatus(t *configv1alpha1.TerragruntStackRun) (metav1.Condition, bool) {
	condition := metav1.Condition{Type: "HasStatus", ObservedGeneration: t.GetGeneration(), Status: metav1.ConditionUnknown, LastTransitionTime: metav1.NewTime(time.Now())}
	if t.Status.State != "" {
...
func GetMaxRetries(defaultValue int, r *configv1alpha1.TerraformRepository, s *configv1alpha1.TerragruntStack) int {
	repo := r.Spec.RemediationStrategy.OnError.MaxRetries
	stack := s.Spec.RemediationStrategy.OnError.MaxRetries
	if repo == nil && stack == nil {
		return defaultValue
	}
	if repo == nil {
		return *stack
...
func (r *Reconciler) HasReachedRetryLimit(run *configv1alpha1.TerragruntStackRun, stack *configv1alpha1.TerragruntStack, repo *configv1alpha1.TerraformRepository) (metav1.Condition, bool) {
	condition := metav1.Condition{Type: "HasReachedRetryLimit", ObservedGeneration: run.GetGeneration(), Status: metav1.ConditionUnknown, LastTransitionTime: metav1.NewTime(time.Now())}
	maxRetries := GetMaxRetries(r.Config.Controller.TerraformMaxRetries, repo, stack)
...
func (r *Reconciler) HasSucceeded(t *configv1alpha1.TerragruntStackRun) (metav1.Condition, bool) {
	condition := metav1.Condition{Type: "HasSucceeded", ObservedGeneration: t.GetGeneration(), Status: metav1.ConditionUnknown, LastTransitionTime: metav1.NewTime(time.Now())}
...
func (r *Reconciler) IsRunning(t *configv1alpha1.TerragruntStackRun) (metav1.Condition, bool) {
	condition := metav1.Condition{Type: "IsRunning", ObservedGeneration: t.GetGeneration(), Status: metav1.ConditionUnknown, LastTransitionTime: metav1.NewTime(time.Now())}
	currentState := t.Status.State
...
func (r *Reconciler) IsInFailureGracePeriod(t *configv1alpha1.TerragruntStackRun) (metav1.Condition, bool) {
	condition := metav1.Condition{Type: "IsInFailureGracePeriod", ObservedGeneration: t.GetGeneration(), Status: metav1.ConditionUnknown, LastTransitionTime: metav1.NewTime(time.Now())}
...
func getLastActionTime(r *Reconciler, run *configv1alpha1.TerragruntStackRun) (time.Time, error) {
