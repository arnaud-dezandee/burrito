...
func TestCreateNetworkMirrorConfig(t *testing.T) {
	t.Run("creates config with network mirror only", func(t *testing.T) {
		...
		if strings.Contains(contentStr, "filesystem_mirror") {
			t.Errorf("config should not contain filesystem_mirror block, got:\n%s", contentStr)
		}
	...
	t.Run("includes filesystem mirror when cache dir is set", func(t *testing.T) {
		saveAndClearEnv(t)
		...
		if !strings.Contains(contentStr, `path = "/tmp/provider-cache"`) {
			t.Errorf("config missing filesystem_mirror path, got:\n%s", contentStr)
		}
		// filesystem_mirror should appear before network_mirror
		fsIdx := strings.Index(contentStr, "filesystem_mirror")
		netIdx := strings.Index(contentStr, "network_mirror")
		if fsIdx > netIdx {
			t.Errorf("filesystem_mirror should appear before network_mirror in config")
		}
	})
